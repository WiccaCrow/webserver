...
CGI::exec(Response *res) {
    ...
    if (childPID < 0) {
        ...
        if (dup2(sock->rdFd(), fileno(stdin)) < 0) {
            Log.syserr() << "CGI::dup2::stdin" << Log.endl;
            exit(123);
        }
        if (dup2(sock->wrFd(), fileno(stdout)) < 0) {
    ...
    if (!body.empty()) {
        if (write(sock->wrFd(), body.c_str(), body.length()) == -1) {
    ...
    g_server->queuePollFd(sock->rdFd(), POLLIN);
    g_server->addClient(sock->rdFd(), client);
